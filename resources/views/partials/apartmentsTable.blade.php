...
<table class="table mt-3">
    ...
    <tbody class="table-group-divider">
        ...
        <tr class="{{ $apartment->visible ? 'table-success' : 'table-danger' }}">
            ...
            <td>
                @php $lastSponsor = ''; @endphp
                @foreach($apartment->sponsors as $sponsor)
                    @if($sponsor->pivot->end_date > $end_date)
                        @php $lastSponsor = $sponsor->pivot->end_date; @endphp
                    @endif
                @endforeach
                @if($lastSponsor != '')
                {{$lastSponsor}}
            ...
            <td>
                <a href="{{ route('apartment.show', $apartment->slug) }}" class="btn btn-primary"><i class="fa-regular fa-eye" style="color: #ffffff;"></i></a>
                <a href="{{ route('apartment.edit', $apartment->slug) }}" class="btn btn-success"><i class="fa-solid fa-pencil" style="color: #ffffff;"></i></a>
                <form action="{{ route('apartment.destroy', $apartment->slug) }}" method="post" class="d-inline-block double-confirm">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-danger"><i class="fa-regular fa-trash-can" style="color: #ffffff;"></i></button>
                </form>
            </td>
        </tr>
        @endforeach
